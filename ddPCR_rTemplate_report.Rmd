...
\fancypagestyle{plain}{%
  ...
  \fancyhf{}%
  \fancyfoot[C]{\footnotesize Page \thepage\, of\, \pageref*{LastPage} `r params$sid` `r params$mid` Generated at `r params$date` by ddPCR MRD Pipeline Version 2.1, 03 Apr 2023}
...
qc.names <- c("Min events per well (>=8000)", "Min FU Alb conc. (copies/uL; >10000, GOOD; 2000-10000; ACCEPTABLE; <2000 POOR)", "Assay controls for marker", "\\hspace{0.5cm}Positive (Dx marker conc., >=10 copies/uL)", "\\hspace{0.5cm}NTC (H2O marker conc., <=0.1 copies/uL)", "Assay Controls for Alb", "\\hspace{0.5cm}Positive (MNC Alb conc., >=50 copies/uL)", "\\hspace{0.5cm}NTC (H2O Alb conc., <=0.1 copies/uL)", "\\textbf{Overall QC result}")
...
if (is.na(dx.baseline)) {
	...
	lineNum <- lineNum + 1
	writeLines(paste(lineNum, ". Using in-plate diagnostic baseline.", sep=""))
} else {
	lineNum <- lineNum + 1
...
lineNum <- lineNum + 1
writeLines(paste(lineNum, ". Using MNC Albumin concentration ", params$mnc.alb.concentration, sep=""))
